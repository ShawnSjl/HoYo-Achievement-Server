...
public class ZzzAchievementController {
    ...
    public SaResult getAllAchievementsWithEmptyRecord() {
        ServiceResponse<List<ZzzAchievementRecordDto>> response = zzzUserRecordService.getAllAchievementsEmptyRecord();
        if (!response.success()) {
            log.error(response.message());
            return SaResult.error("获取ZZZ成就列表失败").setCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
        }
        return SaResult.ok("获取ZZZ成就列表成功").setData(response.data());
    }
